...
public class FrameDataMergeController implements LanguageChangeListener {
    ...
    private boolean allOldSelected;
    private boolean allNewSelected;
    private VideoManagementListController parent;

    public FrameDataMergeController(VideoMergeService viewService, MainViewService mainViewService) {
        this.viewService = viewService;
        this.mainViewService = mainViewService;

        LanguageManager.register(this);

        allOldSelected = false;
        allNewSelected = false;
    }
    ...
    public void initialize() {
        ...
        oldSelectAll.setOnAction(event -> {
            if(!allOldSelected)
                selectAll(oldFramesTable);
            else
                deselectAll(oldFramesTable);
        ...
        newSelectAll.setOnAction(event -> {
            if(!allNewSelected)
                selectAll(newFramesTable);
            else
                deselectAll(newFramesTable);
        ...
        newCheckboxColumn.setCellFactory(CheckBoxTableCell.forTableColumn((Integer index) -> {
            ...
            if(!entry.isHasListener()){
                entry.selectedProperty().addListener((observable, oldValue, newValue) -> entry.setSelected(newValue));
                entry.setHasListener(true);
            }
    ...
    private void selectAll(TableView<TableEntry> framesTable) {
    ...
    private void deselectAll(TableView<TableEntry> framesTable) {
        if(!framesTable.getItems().isEmpty()) {
            for (var e : framesTable.getItems())
                e.setSelected(false);
        }
