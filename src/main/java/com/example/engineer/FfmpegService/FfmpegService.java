...
public class FfmpegService {
    ...
    private static boolean isFfmpegPresent;
    ...
    public FfmpegService() {
        try {
            ...
            isFfmpegPresent = p.waitFor() == 0;
        } catch (Exception e) {
            isFfmpegPresent = false;
        }
    }
    ...
    public byte[] extractFrameBytes(String path, int index) throws InterruptedException, IOException {
        ...
        cmd.add(getFfmpegPath(FFMPEG_PATH));
        cmd.add("-hide_banner");
        cmd.add("-loglevel");
        cmd.add("error");
        cmd.add("-i");
    ...
    private String getFfmpegPath(String path) {
        if (isFfmpegPresent) return path;
